...
#   b = min(255, a + 1)
#   f = x*255.0 - a
#   return [turbo_colormap_data[a][0] + (turbo_colormap_data[b][0] - turbo_colormap_data[a][0]) * f,
...
# We want polynomial maps up to order 10
maxorder    = 10
...
# # turbocolors     = matplotlib.cm.turbo(np.linspace(0,1,256))[:,:3]
# turbocolors     = interpolate(np.linspace(0,1,256))[:,:3]

# from scipy.spatial import KDTree

# import colorsys
# turbocolors_hue = np.asarray([colorsys.rgb_to_hsv(x[0],x[1],x[2]) for x in turbocolors])[:,:1]

# tree    = KDTree(turbocolors_hue)

# tree.query([1.,1.,1.])


# list(matplotlib.cm.hsv((dummy+np.pi)/(2*np.pi)))
...
# Also generate some equally-spaced points for demonstration pruposes
Nr  = 3
gridpoints = None
x = []
y = []
for k in range(Nr):
    ...
    r = k/(Nr-1)
    x += list(np.sin(theta)*r)
    y += list(np.cos(theta)*r)
    
# Convert these to norm samples
norm_demon = np.column_stack((
    np.asarray(x),
    np.asarray(y)))*scale

# Get the target samples
ret_demon = tm.inverse_map(
    Z           = copy.copy(norm_demon))
...
for i in range(ret.shape[0]):
    ...
    # dummy   = colorsys.rgb_to_hsv(dummy[0],dummy[1],dummy[2])

    # _,idx   = tree.query(dummy[0])
    
    # dummy   = turbocolors[idx]
    
    dummy   = turbocolors[int(np.round((dummy+np.pi)/(2*np.pi)*255))]
    
    # _,idx   = tree.query(dummy[:3])
...
whiten  = [list(x)+[0.5] for x in color_values_directional]

# Get the directional color values for the Gaussian samples
cvd_demon = []
for i in range(norm_demon.shape[0]):
    
    # Convert their location relative to the origin to an angle
    dummy   = math.atan2(norm_demon[i,1],norm_demon[i,0])
    dummy   = turbocolors[int(np.round((dummy+np.pi)/(2*np.pi)*255))]
    cvd_demon.append(dummy)

# Convert the list into an array
cvd_demon    = np.asarray(cvd_demon)
...
"""
# Draw a lot of samples from the spiral distribution to get the density contours
X2  = sample_spiral_distribution(100000)
...
xy2 = np.column_stack((
    ...
    np.ndarray.flatten(y2)))
z2  = scipy.stats.multivariate_normal.pdf(
    mean    = np.zeros(2),
    cov     = np.identity(2),
    x       = xy2)
...
plt.contour(x2,y2,z2,levels=10,cmap='Greys')
"""
...
# # Label the axes
# plt.xlabel("dimension $x_{1}$")
# plt.ylabel("dimension $x_{2}$")
...
plt.xlim([-3,3])
# plt.xlabel("marginal $z_{1}$",loc="left")
...
for n in range(norm_demon.shape[0]):
    
    # plt.plot(
    #     [norm_demon[n,0],ret_demon[n,0]],
    #     [norm_demon[n,1],norm_demon[n,1]],
    #     color   = "k")
    ...
    # plt.gca().annotate('', xy=(ret_demon[n,0], norm_demon[n,1]), xytext=(ret_demon[n,0]-(ret_demon[n,0]-norm_demon[n,0])/np.linalg.norm((ret_demon[n,0]-norm_demon[n,0]))*0.05, norm_demon[n,1]), 
    #                     arrowprops=dict(color=cvd_demon[n],headlength=5,headwidth=5,width=1,ls="--"))


# dx  = 1/20
# for prog in np.linspace(0,1,21)[:-1]:
    
#     alpha   = prog*0.5
    
#     for n in range(norm_demon.shape[0]):

#         nx  = norm_demon[n,0] + (ret_demon[n,0]-norm_demon[n,0])*prog
#         ny  = norm_demon[n,1]

#         plt.fill(
#             [nx,nx+dx*(ret_demon[n,0]-norm_demon[n,0]),nx+dx*(ret_demon[n,0]-norm_demon[n,0]),nx],
#             [ny - 0.05,ny - 0.05,ny + 0.05, ny + 0.05],
#             color   = cvd_demon[n],
#             alpha   = alpha,
#             lw      = 0,
#             ec      = "None",
...
# for prog in np.linspace(0,1,21):
...
#     plt.scatter(
#         x       = norm_demon[:,0] + (ret_demon[:,0]-norm_demon[:,0])*prog,
#         y       = norm_demon[:,1],
#         color   = cvd_demon,
#         alpha   = alpha,
#         ec      = "None",
...
# Plot the target distribution's contours
plt.contour(x,y,z,levels=10,cmap='Greys')
"""
...
#     plt.scatter(
#         x       = ret_demon[:,0],
#         y       = norm_demon[:,1] + (ret_demon[:,1]-norm_demon[:,1])*prog,
#         color   = cvd_demon,
#         alpha   = alpha,
#         ec      = "None",
#         s       = 15,
...
for n in range(norm_demon.shape[0]):
    ...
    plt.plot(
        [ret_demon[n,0],ret_demon[n,0]],
        [norm_demon[n,1],ret_demon[n,1]],
        color   = cvd_demon[n],
        ls      = ":")
    
    # plt.gca().annotate('', xy=(ret_demon[n,0], ret_demon[n,1]), xytext=(ret_demon[n,0], ret_demon[n,1]-(ret_demon[n,1]-norm_demon[n,1])/np.linalg.norm((ret_demon[n,1]-norm_demon[n,1]))*0.05), 
    #                     arrowprops=dict(color=cvd_demon[n],headlength=7,headwidth=7,width=1,ls="--"))
...
#     alpha   = prog*0.25 + 0.25
    
#     for n in range(norm_demon.shape[0]):

#         nx  = ret_demon[n,0]
#         ny  = norm_demon[n,1] + (ret_demon[n,1]-norm_demon[n,1])*prog

#         plt.fill(
#             [nx - 0.05,nx + 0.05,nx + 0.05, nx - 0.05],
#             [ny,ny,ny+dy*(ret_demon[n,1]-norm_demon[n,1]),ny+dy*(ret_demon[n,1]-norm_demon[n,1])],
#             color   = cvd_demon[n],
#             alpha   = alpha,
#             lw      = 0,
#             ec      = "None",
...
# plt.gca().annotate('', xy=(-0.075, -0.3), xycoords='axes fraction', xytext=(-0.075, 3.7), 
#                     arrowprops=dict(color='xkcd:silver',headlength=1,headwidth=0,width=1))

# plt.gca().annotate('', xy=(-0.075, 3.7), xycoords='axes fraction', xytext=(1.3, 3.7), 
#                     arrowprops=dict(color='xkcd:silver',headlength=1,headwidth=0,width=1))

# plt.gca().annotate('', xy=(1.3, 3.7), xycoords='axes fraction', xytext=(1.3, -0.3), 
#                     arrowprops=dict(color='xkcd:silver',headlength=1,headwidth=0,width=1))

# plt.gca().annotate('', xy=(1.3, -0.3), xycoords='axes fraction', xytext=(-0.075, -0.3), 
#                     arrowprops=dict(color='xkcd:silver',headlength=1,headwidth=0,width=1))
...
# Plot arrows
arc = np.column_stack((
    -np.sin(np.linspace(0,np.pi,101))*0.07,
    np.linspace(0,2,101)*0.65 ))
arcd = np.diff(arc,axis=0)
...
for a in range(100):
    ...
    if a != 99:
        plt.gca().annotate('', xy=(cx, cy), xycoords='axes fraction', xytext=(cx+tx, cy+ty), 
                    arrowprops=dict(color='xkcd:silver',headlength=1,headwidth=0,width=1))
    else:
        plt.gca().annotate('', xy=(cx+tx, cy+ty), xycoords='axes fraction', xytext=(cx, cy), 
                    arrowprops=dict(color='xkcd:silver',linewidth=2, mutation_scale=25,arrowstyle='->'))
...
# Plot arrows
arc = np.column_stack((
    -np.sin(np.linspace(0,np.pi,101))*0.2,
    np.linspace(0,2,101)*0.65*2 ))
arcd = np.diff(arc,axis=0)
...
for rot in rotdiscretization:
    ...
    # # Then extract the corresponding colour on a circular colormap (hsv)
    # dummy   = list(matplotlib.cm.hsv((rot-rotincrement/2)/(2*np.pi)))
...
# plt.savefig('conditioning_02.png',dpi=600,bbox_inches='tight')
...
whiten  = [list(x)+[0.025] for x in color_values_directional]
...
for i in range(norm_demon.shape[0]):
    ...
    # Convert their location relative to the origin to an angle
    dummy   = math.atan2(norm_demon[i,1],norm_demon[i,0]*0+0.6)
    dummy   = turbocolors[int(np.round((dummy+np.pi)/(2*np.pi)*255))]
...
# plt.gca().text(-0.075, 1.1, '$\mathbf{B}:$ manipulated inversion (conditioning)', transform=plt.gca().transAxes, fontsize=14,
#         verticalalignment='center',horizontalalignment='left',color='xkcd:grey')

plt.gca().text(0.0, 1.05, '$\mathbf{B}:$ conditional inversion', transform=plt.gca().transAxes, fontsize=12,
        verticalalignment='center',horizontalalignment='left',color=[0.2,0.2,0.2])
...
plt.gca().text(0.02, 0.98, 'conditional target distribution', transform=plt.gca().transAxes, fontsize=10,
        verticalalignment='top',horizontalalignment='left',color='xkcd:grey')
...
#     yp  = 1.12 - 0.075*k/100 # 0.115*k/100
    
#     col     = np.asarray(colors.to_rgba('xkcd:grey')) + (np.asarray(colors.to_rgba('xkcd:silver')) - np.asarray(colors.to_rgba('xkcd:grey')))*k/100
    
#     if k < 100:
#         plt.gca().annotate('', xy=(0.47, yp), xycoords='axes fraction', xytext=(0.53, yp), 
#                             arrowprops=dict(color=col,headlength=1,headwidth=0,width=1))
        
# plt.gca().annotate('', xy=(0.5, yp-0.08), xycoords='axes fraction', xytext=(0.5, yp-0.08+0.001), 
#                     arrowprops=dict(color=col,headlength=20,headwidth=35,width=1))

# plt.gca().text(0.55, 1.07, '$S_{2}^{-1}(z_{2};x_{1}^{*})$', transform=plt.gca().transAxes, fontsize=12,
#         verticalalignment='center',horizontalalignment='left',color='xkcd:grey')
...
# plt.gca().annotate('', xy=(-0.075, -0.3), xycoords='axes fraction', xytext=(-0.075, 2.38), 
#                     arrowprops=dict(color='xkcd:silver',headlength=1,headwidth=0,width=1))

# plt.gca().annotate('', xy=(-0.075, 2.38), xycoords='axes fraction', xytext=(1.3, 2.38), 
#                     arrowprops=dict(color='xkcd:silver',headlength=1,headwidth=0,width=1))

# plt.gca().annotate('', xy=(1.3, 2.38), xycoords='axes fraction', xytext=(1.3, -0.3), 
#                     arrowprops=dict(color='xkcd:silver',headlength=1,headwidth=0,width=1))
