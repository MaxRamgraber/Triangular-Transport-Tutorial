...
class transport_map():
    ...
    def reset(self,X):
        ...
        for k in range(self.D):
            ...
            # for i,val in enumerate(self.monotone[k]):
                
            #     val     = [j for j in val if type(j) != str]
                
            #     self.coeffs_mon[k][i] = 10**(-len(val))

            # for i,val in enumerate(self.nonmonotone[k]):
                
            #     val     = [j for j in val if type(j) != str]
                
            #     self.coeffs_nonmon[k][i] = 10**(-len(val))
    ...
    def function_constructor_alternative(self, k = None):
        ...
        for k in Ks:
            ...
            # if len(self.monotone[k]) == 1 and type(self.monotone[k][0]) != str and \
            #     len(self.monotone[k][0]) != 0 and self.monotonicity == 'separable monotonicity':
    ...
    def check_for_special_terms(self):
        ...
        # self.RBF_counter_nm = np.zeros((len(self.nonmonotone),self.X.shape[1]),dtype=int)
    ...
    def calculate_special_term_locations(self,k=None):
    ...
    def worker_task_monotone(self,k,task_supervisor):
        ...
        if self.regularization is None:
            ...
            # der_Psi_mon = copy.copy(self.der_fun_mon[k](
            #         copy.copy(self.X),
            #         self))
        ...
        #     bounds  = bounds,
        #     args    = (A,k),
        #     options = {'disp' : True,'eps' : 1E-12})
        
        # # Retrieve the optimized coefficients
        # coeffs_mon  = copy.copy(opt.x) + self.delta
        
        
        
        
        # coeffs_mon  = self.projectedNewton(
        #     x0      = coeffs_mon, 
        #     fun     = fun_mon_objective, 
        #     args    = (A,k))
        
        # coeffs_mon  += self.delta
        ...
        # print(np.asarray(coeffs_mon).shape)
        # print(coeffs_mon)
    ...
    def objective_function(self, coeffs, k, div = 0):
        ...
        #     nonmonotone_part_der = np.dot(
        #         self.Psi_nonmon[k],
        #         coeffs_nonmon[:,np.newaxis])[...,0]
            
        #     map_der     = monotone_part_der + nonmonotone_part_der
    ...
    def objective_function_jacobian(self, coeffs, k, div = 0):
        ...
        # print(self.fun_mon_strings)
        # print(self.fun_nonmon_strings)
