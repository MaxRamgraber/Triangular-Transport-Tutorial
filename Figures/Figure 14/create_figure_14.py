...
Z   = scipy.stats.norm.rvs(size=(N,2))
...
tm     = transport_map(
    ...
    X                       = copy.copy(Z),         # Training ensemble
...
# Set the coefficients to start from the identity
tm.coeffs_mon[0] *= 0
tm.coeffs_mon[1] *= 0
tm.coeffs_nonmon[0] *= 0
tm.coeffs_nonmon[1] *= 0
tm.coeffs_mon[0][0] = 1
tm.coeffs_mon[1][0] = 1

# Optimize the map
tm.optimize_from_density(
    log_target  = density_wavy_distribution)

# Read out the intermediate optimization results
optimization_callback = tm.optimization_callback

# This map has two map components
K = [0,1]

# This is required to convert the coefficients in the maps-from-densities 
# optimization into the canonical form
divs    = {}
start   = 0
for k in K:
    end     = start + len(tm.nonmonotone[k]) + len(tm.monotone[k])
    divs[k]     = {
        'div'   : len(tm.nonmonotone[k]),
        'start' : start,
        'end'   : end}
    start   = end
...
# Draw a mesh for the density plots
...
# Plot progress at these percentages
percs = [0.,0.015,0.25,1.]

# Create the progress subplots
for idx,perc in enumerate(percs):
    
    plt.subplot(gs[1,idx])
    
    iteration = int(np.round((len(optimization_callback[0])-1)*perc))
    
    # Go through all results and save the coefficients
    for k in K:
        
        # Read out the coefficients
        coeffs = optimization_callback[0][iteration]["coeffs"]
        
        # Save the coefficients
        tm.coeffs_nonmon[k]   = copy.deepcopy(coeffs[divs[k]['start']:divs[k]['start']+divs[k]['div']])
        tm.coeffs_mon[k]      = copy.deepcopy(coeffs[divs[k]['start']+divs[k]['div']:divs[k]['end']])
    
    plt.scatter(Z[::10,0],Z[::10,1],s=1,color="xkcd:dark grey")
    ...
    # Draw the contours
    density = np.exp(density_wavy_distribution(fw) + det).reshape(X_grid.shape)
    ...
    if idx == 0:
        plt.ylabel("pullback density $\mathbf{R}^{\#}\pi$",labelpad=0)
    
    
# Create an empty list for the log densities
logdensities    = []
    
for iteration in range(len(optimization_callback[0])):

    print(iteration)
    
    perc = iteration/(len(optimization_callback[0])-1)
    ...
    # Compute the Jacobian with finite differences
    fw  = copy.copy(tm.map(Z[::10,:]))
    jac     = np.zeros((Z[::10,:].shape[0],2,2))
    det     = np.zeros(Z[::10,:].shape[0])
    ...
    for k in range(2):
        ...
        fw2         = copy.copy(tm.map(Z[::10,:] + offset[np.newaxis,:]))
    ...
    logdensity = np.sum(density_wavy_distribution(fw) + det)
    logdensities.append(logdensity)

plt.subplot(gs[0,:])

maxlength = len(optimization_callback[0])
...
plt.gca().set_ylabel("log-likelihood $\log \mathbf{R}^{\#}\pi(\mathbf{z})$",labelpad=-12)
...
# Save the results
plt.savefig('optimization_from_density_callback.pdf',bbox_inches='tight')
plt.savefig('optimization_from_density_callback.png',bbox_inches='tight')
