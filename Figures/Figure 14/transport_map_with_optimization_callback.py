...
class transport_map():
    ...
    def optimize_from_density(self, log_target):
        
        """
        WARNING: This function should be operational, but is still WIP.
        
        This function optimizes the map's component functions, seeking the
        coefficients which best map the samples to the unnormalized target pdf.
        Note that this is still work in progress. Here, X refers to reference
        samples and Z to pushforward samples.
        
        Variables:
            
            log_target - [required]
                [function] : a function which takes mapped samples as input and 
                returns their unnormalized log target density. 
        ...
        if self.workers > 1 and self.verbose:
            print("WARNING: Optimization from density does not currently support parallelization. Parallelization is ignored.")
            
        # Check for monotonicity type
        if self.monotonicity == "integrated rectifier":
            ...
            start   = 0
            for k in K:
                end     = start + len(self.nonmonotone[k]) + len(self.monotone[k])
                divs[k]     = {
                    'div'   : len(self.nonmonotone[k]),
                    'start' : start,
                    'end'   : end}
                start   = end
                
            # Pre-allocate space for the optimization
            coeffs  = np.zeros(end)
            if self.coeffs_init == 0.:
                coeffs  += 1E-6
            else:
                coeffs  += self.coeffs_init
            
            # Now define the optimization objective
            def objfun(coeffs, log_target, K, divs):
                ...
                # Map the samples
                Z       = np.zeros((self.X.shape[0],self.D))
                ...
                for k in K:
                    ...
                    Z[:,k]  = self.s(
                        x               = self.X, 
                        k               = k, 
                        coeffs_nonmon   = coeffs[divs[k]['start']:divs[k]['end']][:divs[k]['div']], 
                        coeffs_mon      = coeffs[divs[k]['start']:divs[k]['end']][divs[k]['div']:])
                
                # Determine the log target density
                term1   = - log_target(Z)
                
                # Term 2: gradient --------------------------------------------
                
                term2   = np.zeros((self.X.shape[0]))
                ...
                for k in K:
                    ...
                    monotone_part_der = np.dot(
                        self.Psi_mon[k],
                        coeffs[divs[k]['start']:divs[k]['end']][divs[k]['div']:][:,np.newaxis])[...,0]
            ...
            self.optimization_callback[k] = [{"opt": objfun(coeffs,log_target, K, divs),"coeffs": coeffs}]
            
            def callbackF(Xi):
                
                self.optimization_callback[k].append({"opt": objfun(Xi,log_target, K, divs),"coeffs":Xi})
            ...
            for k in K:
                ...
                # Save the coefficients
                self.coeffs_nonmon[k]   = copy.deepcopy(coeffs[divs[k]['start']:divs[k]['start']+divs[k]['div']])
                self.coeffs_mon[k]      = copy.deepcopy(coeffs[divs[k]['start']+divs[k]['div']:divs[k]['end']])
        ...
        # Check for monotonicity type
            ...
            # Assign the optimization constraints
            optimization_constraints_lb     = -np.ones(end)*np.inf
            optimization_constraints_ub     = +np.ones(end)*np.inf
            
            for k in K:
                
                # Constrain the monotone coefficients bound to be zero
                optimization_constraints_lb[divs[k]['start']+divs[k]['div']:divs[k]['end']] = 0
            
            # Specify the optimization bounds
            bounds      = []
            for idx in range(len(optimization_constraints_lb)):
                bounds.append(
                    [optimization_constraints_lb[idx],
                     optimization_constraints_ub[idx]])
            ...
            coeffs = np.asarray(
                list(self.coeffs_nonmon[0]) +\
                list(self.coeffs_mon[0]) +\
                list(self.coeffs_nonmon[1]) +\
                list(self.coeffs_mon[1])
                )
            # # print(coeffs.shape)
            # print(self.coeffs_mon[0].shape)
            # print(self.coeffs_mon[1].shape)
            # print(self.coeffs_nonmon[0].shape)
            # print(self.coeffs_nonmon[1].shape)
            ...
            def objfun(coeffs, log_target, K, divs):
                ...
                # Pre-allocate space for the second term
                term2   = np.zeros(self.X.shape[0])
                
                # Draw Evaluate the map gradient
                for k in K:
                    
                    # Calculate the map gradient
                    term2   -= np.log(np.dot(
                        self.der_Psi_mon[k],
                        coeffs[divs[k]['start']:divs[k]['end']][divs[k]['div']:][:,np.newaxis])[:,0])
            ...
            self.optimization_callback[0] = [{"opt": objfun(coeffs,log_target, K, divs),"coeffs": coeffs}]
            ...
            def callbackF(Xi):
                ...
                self.optimization_callback[0].append({"opt": objfun(Xi,log_target, K, divs),"coeffs":Xi})
    ...
    def worker_task_monotone(self,k,task_supervisor):
        ...
        self.optimization_callback[k] = [{"opt": fun_mon_objective(coeffs_mon,A,k),"coeffs":[coeffs_mon,coeffs_nonmon]}]
        
        def callbackF(Xi):
            print(fun_mon_objective(Xi,A,k))
            ...
            self.optimization_callback[k].append({"opt": fun_mon_objective(Xi,A,k),"coeffs":[Xi,cnm]})
