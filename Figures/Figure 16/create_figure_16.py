...
color_values_directional_reference = []
for i in range(norm_samples.shape[0]):
    ...
    color_values_directional_reference.append(dummy)
    

# Convert the list into an array
color_values_directional_reference    = np.asarray(color_values_directional_reference)
...
xy  = np.column_stack((
    np.ndarray.flatten(xg),
    np.ndarray.flatten(yg)))
...
plt.contour(xg,yg,z,levels=5,cmap='Greys_r')
...
plt.title("conditional "+"$\pi(x_2|x_1^{*})$", fontsize = 10)

plt.text(
    0.5,
    1.2,
    "$\mathbf{A}:$ analytic solution",
...
ret_standard    = tm.inverse_map(
    X_star      = norm_samples[:,0][:,np.newaxis]*0+1.5,
    Z           = norm_samples[:,1][:,np.newaxis])[:,1]

ret_composite   = tm.inverse_map(
    X_star      = norm_samples[:,0][:,np.newaxis]*0+1.5,
    Z           = pushforward[:,1][:,None])[:,1]
...
plt.gca().annotate('', xy=(-0.26+1.1, 0.5), xycoords='axes fraction', xytext=(-0.26+1.2+0.001, 0.5), 
                    arrowprops=dict(color="xkcd:dark grey",headlength=20,headwidth=35,width=1),)

for i in range(25):
    
    plt.gca().annotate('', xy=(-i/25*0.2+1.2, 0.48), xycoords='axes fraction', xytext=(-i/25*0.2+1.2, 0.52), 
            arrowprops=dict(color="xkcd:dark grey",arrowstyle="-",lw=20),)
...
plt.scatter(ret[::10,0],ret[::10,1],facecolor = "xkcd:grey", edgecolor = "None", zorder = 10, s = 10)

plt.scatter(ret[::10,0]*0+1.5,ret_standard[::10],facecolor = color_values_directional_reference[::10], edgecolor = "None", zorder = 100, s = 10)
...
plt.title("conditional samples "+"$x_2^{*}$", fontsize = 10)
...
for bni,bn in enumerate(bins):
    for j,ind in enumerate(indices[bni]):
        ...
        plt.fill(
            ...
            color = color_values_directional_reference[::10][ind],#"xkcd:grey",
            ec      = "None",
            zorder  = 10)
...
plt.scatter(norm_samples[::10,0],norm_samples[::10,1],facecolor = "xkcd:grey", edgecolor = "None", zorder = 10, s = 10)

plt.scatter(norm_samples[::10,0]*0+1.5,norm_samples[::10,1],facecolor = color_values_directional_reference[::10], edgecolor = "None", zorder = 100, s = 10)

plt.title("samples "+"$(x_1^{*},z_{2})$", fontsize = 10)
...
gs_mid = gridspec.GridSpecFromSubplotSpec(
    ...
    wspace          = 0.1, 
    hspace          = 0.0,
    subplot_spec    = gs_supra[0,1])

gs_sub  = matplotlib.gridspec.GridSpecFromSubplotSpec(
    nrows           = 2,
    ncols           = 2,
    width_ratios    = [1.,0.2],
    height_ratios   = [1.,0.2],
...
for i in range(25):
    ...
    plt.gca().annotate('', xy=(-i/25*0.1, 0.48), xycoords='axes fraction', xytext=(-i/25*0.1, 0.52), 
            arrowprops=dict(color="xkcd:dark grey",arrowstyle="-",lw=20),)

plt.scatter(X[::10,0],pushforward[::10,1],facecolor = color_values_directional[::10], edgecolor = "None", zorder = 100, s = 10)

plt.title("transform to "+r"$(x_1,\tilde{z}_{2})$", fontsize = 10)
...
plt.text(
    -0.05,
    1.2,
    "$\mathbf{C}:$ composite map inverse",
    transform = plt.gca().transAxes,
    color = "k",
    ha = "center",
    va = "center")   
...
for i in range(100):
    ...
    plt.gca().annotate('', xy=(1.08,-i/100*0.7), xycoords='axes fraction', xytext=(1.12,-i/100*0.7), 
            arrowprops=dict(color="xkcd:dark grey",arrowstyle="-",lw=20),)
...
plt.title("target samples "+"$(x_1,x_2)$", fontsize = 10)
...
plt.scatter(X[::10,0],X[::10,1],facecolor = "xkcd:grey", edgecolor = "None", zorder = 10, s = 10)

plt.scatter(X[::10,0]*0+1.5,ret_composite[::10],facecolor = color[::10], edgecolor = "None", zorder = 100, s = 10)
...
plt.scatter(pushforward[::10,0],pushforward[::10,1],facecolor = "xkcd:grey", edgecolor = "None", zorder = 10, s = 10)

plt.scatter(pushforward[::10,0]*0+1.5,pushforward[::10,1],facecolor = color[::10], edgecolor = "None", zorder = 100, s = 10)

plt.title(r"replace $x_1$ with $x_1{*}$", fontsize = 10)
...
plt.gca().annotate('', xy=(0.5, 0.845), xycoords='axes fraction', xytext=(0.5, 0.845+0.001), 
                    arrowprops=dict(color="xkcd:dark grey",headlength=20,headwidth=35,width=1),)
...
plt.savefig("composite_02.png",dpi=300,bbox_inches="tight")
plt.savefig("composite_02.pdf",dpi=300,bbox_inches="tight")