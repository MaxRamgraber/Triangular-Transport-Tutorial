...
turbocolors_hex = [colors.to_hex(plt.get_cmap("turbo")(i)) for i in np.linspace(0,1,256)]
...
def sample_wavy_distribution_rotated(size):
    ...
    # For the rotated version, just switch x and y
    X2  = copy.copy(X)
    X[:,0] = X2[:,1]
    X[:,1] = X2[:,0]
...
plt.close('all')
plt.figure(figsize=(15,12)) # 14,21
...
for di,dirs in enumerate(['map_fidelity_wavy','map_fidelity_wavy_rotated','map_fidelity_corner']):
    
    current_directory = root_directory + '//' + dirs
    ...
    if di == 0:
        ...
        X = sample_wavy_distribution_rotated(N)
    else:
    ...
    gs2 = gridspec.GridSpecFromSubplotSpec(
        ...
        wspace          = 0.0, 
        hspace          = 0.0,
        subplot_spec    = gs[di,0])
    
    plt.subplot(gs2[0,0])
    
    if di == 0:
        plt.title("training samples",loc="left")
    
    # Then scatter the colored samples above the contours
    plt.scatter(
        X[:,0]*X_std[0]+X_mean[0],
        X[:,1]*X_std[1]+X_mean[1],
        s       = 1,
        color   = [0.2,0.2,0.2])
    ...
    indices = []
    for ind in [np.where(np.logical_and(ret[:,0] >= bn[0],ret[:,0] < bn[1])) for bn in bins]:
        indices.append(ind[0][np.flip(np.argsort(ret[:,1][ind]))])
    ...
    if di == 0:
        plt.ylabel("distribution: $\mathbf{wavy}$", fontsize = 12,loc="bottom")
    elif di == 1:
        plt.ylabel("distribution: $\mathbf{wavy}$ $\mathbf{rotated}$", fontsize = 12,loc="bottom")
    elif di == 2:
        plt.ylabel("distribution: $\mathbf{corner}$ $\mathbf{multimodal}$", fontsize = 12,loc="bottom")
    ...
    # Get the indices in the bin, sort them by their y position
    indices = []
    for ind in [np.where(np.logical_and(ret[:,1] >= bn[0],ret[:,1] < bn[1])) for bn in bins]:
        indices.append(ind[0][np.argsort(ret[:,0][ind])])
    ...
    for complexity in [0,1,2]:
        ...
        # Train the map
        if dirs+'_dict_coeffs_order='+str(maxorder)+'_complexity='+str(complexity)+'.p' not in os.listdir(root_directory):
            ...
            dct = {
                ...
                'X_mean'        : tm.X_mean ,
                'X_std'         : tm.X_std }
            pickle.dump(dct,open(dirs+'_dict_coeffs_order='+str(maxorder)+'_complexity='+str(complexity)+'.p','wb'))
            
        else:
            
            print('Pre-optimized coefficients found. Extracting...')
            
            dct = pickle.load(open(dirs+'_dict_coeffs_order='+str(maxorder)+'_complexity='+str(complexity)+'.p','rb'))
            
            tm.coeffs_mon       = copy.copy(dct['coeffs_mon'])
            tm.coeffs_nonmon    = copy.copy(dct['coeffs_nonmon'])
            tm.X_mean           = copy.copy(dct['X_mean'])
            tm.X_std            = copy.copy(dct['X_std'])

        # Let's see if tm can map standard Gaussian samples to the target
        norm_samples    = scipy.stats.norm.rvs(size=(N,2))
        ...
        gs2 = gridspec.GridSpecFromSubplotSpec(
            ...
            wspace          = 0.0, 
            hspace          = 0.0,
            subplot_spec    = gs[di,complexity+1])
        ...
        num_coeffs = 0
        num_coeffs += len(nonmonotone[0])
        num_coeffs += len(monotone[0])
        num_coeffs += len(nonmonotone[1])
        num_coeffs += len(monotone[1])
        ...
        if di == 0 and complexity == 0:
            plt.title("$\mathbf{marginal}$ map",loc="left")
        elif di == 0 and complexity == 1:
            plt.title("$\mathbf{separable}$ map",loc="left")
        elif di == 0 and complexity == 2:
            plt.title("$\mathbf{cross}$-$\mathbf{term}$ map",loc="left")
        
        
        Xg,Yg   = np.meshgrid(
            np.linspace(-3,3,101)[1:],
            np.linspace(-3,3,101)[1:])
        ...
        dx      = np.sort(np.unique(Xg))
        dx      = dx[1]-dx[0]
        
        dy      = np.sort(np.unique(Yg))
        dy      = dy[1]-dy[0]
...
os.chdir(root_directory)
plt.savefig('map_types_separable_crossterm_maxturbo_num_coeffs.pdf',dpi=600,bbox_inches='tight')
plt.savefig('map_types_separable_crossterm_maxturbo_num_coeffs.png',dpi=600,bbox_inches='tight')
plt.savefig('map_types_separable_crossterm_maxturbo_num_coeffs_smaller.png',dpi=300,bbox_inches='tight')