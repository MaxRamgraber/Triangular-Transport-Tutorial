...
plt.close('all')
plt.figure(figsize=(13,11)) # 14,21
...
for complexity in [0,1,2]:
    ...
    ret         = tm.inverse_map(norm_samples)
    
    # plt.subplot(gs[complexity,0])
    ...
    plt.scatter(
        x       = ret[:,0],
        y       = ret[:,1],
        color   = [0.2,0.2,0.2],#color_values_directional,
        s       = 1,
        zorder  = 10)
    
    for idx,condval in enumerate(condvals):
        
        plt.gca().text((condval+3)/6, 1.035, '$x_{1}^{*'+symbol[idx]+'}$', transform=plt.gca().transAxes,
            color = "#1988B8", verticalalignment='bottom',horizontalalignment='center',
            fontsize = 12)
        
        line    = np.linspace(-3,3,101)[:-1]
        ldif    = line[1]-line[0]
        for x in line:
    
            # Convert their location relative to the origin to an angle
            dummy   = math.atan2(x,condval)
    ...
    if complexity == 0:
        plt.ylabel("$\mathbf{marginal}$ map",loc="bottom")
    elif complexity == 1:
        plt.ylabel("$\mathbf{separable}$ map",loc="bottom")
    else:
        plt.ylabel("$\mathbf{cross}$-$\mathbf{term}$ map",loc="bottom")
    ...
    for idx,condval in enumerate(condvals):
        ...
        retcond = tm.inverse_map(
            X_star      = np.ones((N,1))*condval,
        ...
        gs2 = gridspec.GridSpecFromSubplotSpec(
            ...
            subplot_spec    = gs[complexity,1+idx])
    
    
        # Get the directional color values for the Gaussian samples
        color_values_directional_normal = []
        for i in range(norm_samples.shape[0]):
            
            # Convert their location relative to the origin to an angle
            dummy   = math.atan2(norm_samples[i,1],condval)
            ...
            color_values_directional_normal.append(dummy)

        # Apply the inverse map
        f_x     = np.linspace(-3,3,1001)[:,np.newaxis]
        f_y     = tm.inverse_map(
            X_star      = np.ones((1001,1))*condval,
            Z           = f_x)[:,1]
        ...
        for n in np.arange(0,N,10):
            ...
            plt.plot(
                [norm_samples[n,1],norm_samples[n,1],3],
                [3,retcond[n,1],retcond[n,1]],
                color   = color_values_directional_normal[n],
                alpha   = 0.5,
                lw      = 0.2,
                zorder  = 0)
        ...
        for bni,bn in enumerate(bins):
            for j,ind in enumerate(indices[bni]):
                ...
                plt.fill(
                    ...
                    [j,j,j+1,j+1],
                    color = color_values_directional_normal[ind])
        ...
        plt.gca().text(0.935, 0.75, '$z_{2}$', transform=plt.gca().transAxes,
            color = "#01A109", verticalalignment='center',horizontalalignment='center',
            fontsize = 12)
        ...
        plt.gca().text(0.6, 0.075, '$x_{2}^{*'+symbol[idx]+'}$', transform=plt.gca().transAxes,
            color = "#FF5000", verticalalignment='center',horizontalalignment='center',
            fontsize = 12)
        
        
        plt.subplot(gs2[0,1])
        
        plt.gca().text(0.5, 0.8, 'for', transform=plt.gca().transAxes,
            color = "#1988B8", verticalalignment='center',horizontalalignment='center',
            fontsize = 8)
        plt.gca().text(0.5, 0.35, '$x_{1}^{*'+symbol[idx]+'}$', transform=plt.gca().transAxes,
            color = "#1988B8", verticalalignment='center',horizontalalignment='center',
            fontsize = 12)
        
        plt.axis("off")
        
plt.savefig('summary.png',dpi=600,bbox_inches='tight')
plt.savefig('summary.pdf',dpi=600,bbox_inches='tight')
